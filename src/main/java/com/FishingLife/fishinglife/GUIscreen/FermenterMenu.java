...
public class FermenterMenu extends AbstractContainerMenu {
    ...
    public FermenterMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(iItemHandler -> {
            this.addSlot(new SlotItemHandler(iItemHandler, 0, 61, 11));
            this.addSlot(new SlotItemHandler(iItemHandler, 1, 100, 11));
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, FishingLifeBlocksRegistry.FERMENTER.get());
    }
